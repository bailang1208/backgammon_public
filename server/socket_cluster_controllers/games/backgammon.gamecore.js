...
function validateMove(moves, gameStatus, socket, real_player_turn) {
	...
	var player1_id = gameStatus.creator.userId;
	var player2_id = gameStatus.partner.userId;
    var userId = commonCtrl.getUserId(socket);
	var player_turn 	= player1_id == userId ? 1 : 2;
	
	if( real_player_turn == player_turn/* && $room['player_last']!=$player_turn && $room['step']==2 */) {
		var dice = {1: gameStatus.zar.creator, 2: gameStatus.zar.partner};
